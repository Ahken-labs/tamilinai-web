...
function ReceivedBadge() {
  return (
    <div className="flex px-1 items-center rounded-[28.5px] bg-[#B31B38]">
      <InterestArrowIcon className="w-6 h-6 rotate-450" />
    </div>
  );
}
...
function ActionButton({ action }: { action: ActionConfig }) {
  const inner = (
    <>
      {action.showDot && <RedDotIcon className="w-2.5 md:w-3 h-2.5 md:h-3" />}
      {action.label}
      <ChevronRightIcon className="w-4 sm:w-5 md:w-5.5 lg:w-6 h-4 sm:h-5 md:h-5.5 lg:h-6 shrink-0 text-[#B31B38]" />
    </>
  );
  return action.href ? (
...
export default function InterestCard({ interest, isLast = false }: InterestCardProps) {
  ...
  return (
    <div
      className={`flex items-center gap-3 md:gap-4 px-3 md:px-4 py-4 md:py-6 bg-white ${!isLast ? "border-b border-[#EAEAEA]" : ""}`}
    >
      {/* Photo */}
