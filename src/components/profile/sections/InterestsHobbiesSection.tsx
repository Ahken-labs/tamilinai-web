...
export default function InterestsHobbiesSection() {
  ...
  return (
    <div className="pt-3 md:pt-4 font-poppins">
      ...
      <button
        type="button"
        onClick={() => { setDraft(selected); setSearch(""); setOpen(true); }}
        className="inline-flex max-w-full flex-wrap items-center gap-2 rounded-[48px] border border-[rgba(179,27,56,0.25)] bg-[#FFF0F3] px-3 py-1 text-left"
      >
        <span className="font-16 text-[#656565] leading-[150%]">
          {selected.length > 0 ? "Edit interests" : "Add interests & hobbies"}
        </span>
        ...
        <ChevronRightIcon className="h-3 w-3 sm:h-3.3 sm:w-3.3 md:h-4 md:w-4 shrink-0" />
          ...
          selected.map((item) => (
            <span
              key={item}
              className="inline-flex items-center rounded-[48px] border border-[rgba(179,27,56,0.25)] bg-[#FFF0F3] px-3 py-1 px-3 py-1 font-16 text-[#656565] leading-[150%]"
            >
              {item}
            </span>
          ))}
          ...
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 p-4">
            <div className="flex max-h-[90vh] w-full max-w-[920px] flex-col overflow-hidden rounded-[16px] bg-white shadow-2xl">
              <div className="shrink-0 px-4 pt-4 pb-2 md:px-5 md:pt-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    <h2 className="font-24 font-semibold leading-[150%] text-dark">
                      Interests & Hobbies
                    </h2>
                    <p className="mt-1 font-16 font-normal leading-[150%] text-dark">
                      Add your hobbies to find matches with similar interests.
                    </p>
                  </div>
              ...
              <div className="flex-1 overflow-y-auto border-t border-[#EAEAEA] px-4 py-3 md:px-5 md:py-4">
                {draft.length > 0 && (
                  <div className="flex flex-wrap gap-2 md:gap-3">
                    {draft.map((item) => (
                      ...
                      >
                        <span className="font-16 font-semibold leading-[125%] text-[#656565]">
                          {item}
                        </span>
                        <span className="text-secondary3 text-[16px] leading-none">
                          ×
                        </span>
                      </button>
                    ))}
                ...
                <div className="mt-4 flex items-center gap-2 rounded-[41px] bg-[#E0E0E0] px-2 py-2">
                  ...
                  <input
                    ...
                    className="flex-1 bg-transparent font-16 font-normal leading-[150%] text-[#656565] outline-none placeholder:text-[#656565]"
                  />
                  {search && (
                    <button
                      type="button"
                        ...
                        {group.items.map((item) => {
                          ...
                          return (
                            <button
                              ...
                              type="button"
                              onClick={() => toggleItem(item)}
                              className={`rounded-[28px] px-3 py-2.5 font-16 font-normal leading-[125%] transition-colors ${active
              ...
              <div className="flex justify-end gap-4 md:gap-5 border-t border-[#EAEAEA] px-4 md:px-5 py-3 md:py-4 shrink-0">
                ...
                <Button text="Add interests & hobbies" onPress={handleConfirm}
                  className="w-full"
                />
              </div>
