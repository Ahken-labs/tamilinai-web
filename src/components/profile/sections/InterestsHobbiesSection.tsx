...
export function HobbiesPopup({ initialSelected, onClose, onConfirm }: {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-end min-[500px]:items-center justify-center min-[500px]:p-4 bg-black/50">
      <div className="flex max-h-[90dvh] min-[500px]:max-h-[90vh] w-full min-[500px]:max-w-[920px] flex-col overflow-hidden rounded-t-[16px] min-[500px]:rounded-[16px] bg-white shadow-2xl">
        <div className="shrink-0 px-4 pt-4 pb-2 md:px-5 md:pt-5">
          <div className="flex items-start justify-between gap-4">
            ...
            <button type="button" onClick={onClose} className="shrink-0 cursor-pointer" aria-label="Close"><CloseCircleIcon /></button>
          </div>
        </div>
        <div className="flex-1 overflow-y-auto border-t border-[#EAEAEA] max-[500px]:px-0 px-4 md:px-5 max-[500px]:py-4 py-3 md:py-4 pb-[72px] min-[500px]:pb-3 md:pb-4">
          {draft.length > 0 && (
            <div ref={tagsRowRef} className="no-scrollbar flex max-[500px]:flex-nowrap max-[500px]:overflow-x-auto flex-wrap gap-2 md:gap-3 mb-4">
              {draft.map(item => (
                <button key={item} type="button" onClick={() => toggle(item)} className="flex shrink-0 items-center gap-2 rounded-[28px] border border-[rgba(179,27,56,0.25)] bg-[#FFF0F3] px-3 py-2.5 cursor-pointer">
              ...
              <div key={group.heading} className="flex flex-col gap-3 md:gap-4">
                ...
                <div className="flex flex-wrap gap-2 md:gap-3">
                  {group.items.map(item => {
                    ...
                    return <button key={item} type="button" onClick={() => toggle(item)} className={`rounded-[28px] px-3 py-2.5 text-[16px] font-normal leading-[125%] transition-colors cursor-pointer ${active ? "border border-[rgba(179,27,56,0.25)] bg-[#FFF0F3] text-[#222]" : "bg-[#F0F0F0] text-[#656565] hover:bg-[#EAEAEA]"}`}>{item}</button>;
                  })}
                </div>
      ...
      <div className="min-[500px]:hidden fixed bottom-0 left-0 right-0 px-4 py-2 z-10" style={{ background: "rgba(255, 255, 255, 0.60)", backdropFilter: "blur(11px)" }}>
        <Button text="Add interests & hobbies" onPress={() => { onConfirm(draft); onClose(); }} className="!w-full" />
      </div>
    </div>,
    document.body
  ...
  return (
    <div className="pt-3 md:pt-4 font-poppins">
      <div className="flex flex-wrap gap-2">
        <button type="button" onClick={() => { setDraft(selected); setOpen(true); }} className="inline-flex max-w-full flex-wrap items-center gap-2 rounded-[48px] border border-[rgba(179,27,56,0.25)] bg-[#FFF0F3] px-3 py-1 text-left">
      ...
      {open && (
        <HobbiesPopup initialSelected={draft} onClose={() => setOpen(false)} onConfirm={items => { setSelected(items); setDraft(items); setOpen(false); try { sessionStorage.setItem(KEY, JSON.stringify({ hobbies: items })); onDirty(); } catch {} }} />
      )}
    </div>
  );
}
