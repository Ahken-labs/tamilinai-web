...
import { submitProfileSetup, savePartnerPreferences } from "../../lib/api/user";
...
export default function PhotoUploadForm() {
  ...
  const [aboutPartnerError, setAboutPartnerError] = useState<{ message: string; offendingWord: string } | null>(null);
  ...
  function submitSetup(skipPhoto: boolean) {
    ...
    router.replace("/upgrade?from=setup");

    // Fire in background — user is already navigating forward
    submitProfileSetup(fd)
      .then(() => clearDraftData())
      .catch(() => {});
    if (aboutPartner.trim() && !aboutPartnerError) {
      savePartnerPreferences({ aboutPartner: aboutPartner.trim() }).catch(() => {});
    }
  }
  ...
  return (
    <>
      ...
      <FormCardLayout
        bgColor="bg-light"
        paddingHorizontal="px-6 sm:px-6"
        childrenTopMargin="pt-4 sm:pt-8"
        paddingBottom="max-[500px]:pb-0 pb-4 sm:pb-8"
        footer={
          <>
      ...
      >
        ...
        <h1 className="font-24 min-[500px]:mt-10 font-semibold text-dark leading-[150%] max-[500px]:text-left text-center">
          {t("Add_your_clear_photo")}
        </h1>

        {/* Desktop subtitle */}
        <p className="hidden max-w-[410px] mx-auto min-[500px]:block mt-2 font-16 font-normal text-dark leading-[150%] text-center">
          {t("Photo_subtitle")}
        </p>

        {/* Photo circle — shared */}
        <div className="mt-6 sm:-8 md:mt-10 lg:mt-12 flex flex-col min-[500px]:flex-row items-center justify-center gap-5 sm:gap-7 md:gap-10">
        ...
        <div className="mt-8 sm:mt-10 md:mt-12">
          <div className="flex items-center justify-between">
            <p className="min-[360px]:flex hidden text-[16px] font-medium text-dark">{t("About_me")}</p>
            <p className="max-[360px]:flex hidden text-[16px] font-medium text-dark">{t("AboutMe")}</p>
          ...
          <textarea
            ...
            className={`w-full mt-3 rounded-[12px] border text-[14px] md:text-[16px] font-normal text-dark placeholder:text-[#656565] resize-none outline-none transition-colors bg-[#F2F2F2] max-[500px]:h-[123px] h-[160px] md:h-[199px] p-3 sm:p-4 ${aboutMeError ? "border-[#B31B38]" : "border-[#F2F2F2]"}`}
          />
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between">
            <p className="text-[16px] font-medium text-dark">{t("About_my_partner")}</p>
          </div>
          ...
          <textarea
            ...
            onChange={(e) => {
              ...
              if (countWords(val) <= MAX_WORDS) {
                setAboutPartner(val);
                setAboutPartnerError(validateAboutMe(val));
              }
            }}
            placeholder={t("About_me_placeholder")}
            className={`w-full mt-3 rounded-[12px] border text-[14px] md:text-[16px] font-normal text-dark placeholder:text-[#656565] resize-none outline-none transition-colors bg-[#F2F2F2] max-[500px]:h-[123px] h-[160px] md:h-[199px] p-3 sm:p-4 ${aboutPartnerError ? "border-[#B31B38]" : "border-[#F2F2F2]"}`}
          />
          <div className="flex text-[12px] md:text-[14px] justify-between mt-1 gap-2">
            <span className={aboutPartnerError ? "text-[#B31B38] font-medium flex-1" : "text-secondary4 flex-1"}>
              {aboutPartnerError
                ? splitHighlight(aboutPartnerError.message, aboutPartnerError.offendingWord).map((seg, i) =>
                  seg.highlight
                    ? <span key={i} className="underline decoration-[0.5px] underline-offset-2">{seg.text}</span>
                    : <span key={i}>{seg.text}</span>
                )
            ...
            <span className="text-secondary4 shrink-0">{countWords(aboutPartner)} {t("Word_count")}</span>
          </div>
        </div>
        ...
        <div className="min-[500px]:hidden mt-6 flex flex-col gap-4">
          <p className="font-16 font-normal text-dark leading-[150%]">{t("Photo_subtitle")}</p>
          ...
          <div className="flex max-[500px]:gap-2 gap-3 items-start">
            <button className="max-[500px]:mt-1" onClick={() => { setAgreed(prev => !prev); setShowError(false); }}>
              <CheckboxIcon checked={agreed} />
            </button>
